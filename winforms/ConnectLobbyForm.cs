...
namespace GenshinQuartetPlayer2.winforms
{
    public partial class ConnectLobbyForm : Form
    {
        ...
        {
            if (offsetTextBox.Text == "" || nameTextBox.Text == "" || portTextBox.Text == "" || ipAddressTextBox.Text == "" || !int.TryParse(offsetTextBox.Text, out _) || !int.TryParse(portTextBox.Text, out _))
            {
                connectButton.Enabled = false; // todo сделать проверку на правильность ip
            }
